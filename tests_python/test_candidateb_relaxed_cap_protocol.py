...
def test_relaxed_cap_uses_matched_10k_protocol():
    ...
    assert mod._ABLATION_STEP == 5_000

    assert mod._FAMINE_OVERRIDES["environment_shift_step"] == 3_000
    assert mod._BOOM_BUST_OVERRIDES["environment_cycle_period"] == 2_500
    assert mod._SEASONAL_OVERRIDES["environment_cycle_period"] == 1_000
