...
\paragraph{Selection differential.}
To test whether evolution drives directed trait change rather than neutral
drift, we track organism-level physiology across lineage generations
($n$=15 seeds, 10,000 steps; Figure~\ref{fig:trait_evolution}).
Organisms are stratified by generation quartile at the final step:
high-generation organisms have undergone more reproductive selection events
than low-generation organisms in the same population.
In evolved populations, high-generation organisms show significantly higher
energy than low-generation organisms
(Cliff's $\delta$=\textbf{[X]}, $p$=\textbf{[Y]}),
whereas no-evolution controls show no such stratification
($\delta{\approx}$\textbf{[Z]}, $p$$\approx$1)---generation count in
no-evolution populations reflects demographic structure, not accumulated
selection.
Mean energy also diverges monotonically between evolved and no-evolution
populations across all snapshot steps.
These results confirm that evolution in this system produces measurable
directional adaptation rather than genetic drift alone, even within the
10,000-step window.
...
\caption{Selection differential evidence ($n$=15 seeds, 10,000 steps).
(A)~Mean energy trajectory for evolved (solid) and no-evolution (dashed)
conditions, $\pm$1 SEM (seed-level).
(B)~Per-seed high-generation vs.\ low-generation organism energy at
step 10,000, for evolved and no-evolution populations.
High-generation organisms accumulate energy advantage only under selection.}
