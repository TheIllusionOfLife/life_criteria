"""Candidate B (kin-sensing) stress-test experiment — harsh perturbation regimes.

Two regimes test whether collective sensing (self/non-self recognition via
kin_fraction) provides survival benefit under genuine stress:

Famine
------
  Sharp resource crash at step 3,000.  Same calibrated parameters as
  criterion8 stress-test (PR #9): ~60% baseline extinction.

Boom-Bust
---------
  Cyclic resource availability with period 2,500 (2 bust phases in 10k steps).
  Same calibrated parameters as criterion8 stress-test (PR #9).

Conditions
----------
  baseline              : 7-criteria system, collective sensing disabled
  candidateB_on         : collective sensing enabled (kin_fraction → NN ch8)
  candidateB_ablated    : sensing enabled but ablated mid-run at step 5,000
  sham                  : compute-matched permuted kin_fraction (breaks causality)
...
Usage
-----
    uv run python scripts/experiment_candidateB_stress.py --regime famine --tier calibration
    uv run python scripts/experiment_candidateB_stress.py --regime boom_bust --tier 1 --workers 4
"""
...
# ---------------------------------------------------------------------------
# Perturbation regime configs (same calibrated params as criterion8 stress-test)
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Candidate B stress-test — famine/boom-bust perturbation regimes.",
...
def main() -> None:
    ...
    log(f"Candidate B stress-test — Regime: {regime}, Tier: {args.tier}")
    ...
    filename_prefix = f"candidateB_{regime}_"
