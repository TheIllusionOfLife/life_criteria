"""Candidate B (kin-sensing) stress-test analysis — statistical comparison.

Reads
-----
    experiments/candidateB_{regime}_{condition}.json  (for each regime × condition)

Writes
------
    experiments/candidateB_stress_analysis.json   — per-regime statistics
    experiments/candidateB_stress_manifest.json   — run metadata for reproducibility

Metrics
-------
  Survival AUC            : sum(alive_count) over all samples
  Post-shock AUC          : sum(alive_count) for steps >= shock_step (famine)
  Post-shock survival rate: alive at final / alive at shock
  Recovery time           : first step where alive_count stabilises
  ...
  Kin-fraction trajectory : mean kin_fraction_mean over time (diagnostic)

Usage
-----
    uv run python scripts/analyze_candidateB_stress.py
    uv run python scripts/analyze_candidateB_stress.py --no-figure
"""
...
_ANALYSIS_OUT = _EXP_DIR / "candidateB_stress_analysis.json"
_MANIFEST_OUT = _EXP_DIR / "candidateB_stress_manifest.json"
...
def _learning_curve_slope(per_cycle: list[float]) -> float | None:
    """OLS slope of per-cycle survival vs cycle number."""
    n = len(per_cycle)
    if n < 2:
        return None
...
def _kin_fraction_trajectory(result: dict) -> list[float]:
    """Extract kin_fraction_mean over time for a single seed."""
...
def _analyze_famine(loaded: dict[str, list[dict]]) -> dict:
    ...
    for cond in _CONDITIONS:
        ...
        summaries[cond] = {
            ...
            "kin_fraction_final": {
                "per_seed": kf_final,
                "mean": float(np.mean(kf_final)),
            },
        }

    # Pairwise comparisons vs baseline (on post-shock AUC)
...
def _analyze_boom_bust(loaded: dict[str, list[dict]]) -> dict:
    ...
    for cond in _CONDITIONS:
        ...
        # Kin-fraction diagnostics
        kf_final = [
            r["samples"][-1].get("kin_fraction_mean", 0.0) if r.get("samples") else 0.0
            for r in results
        ]
    ...
    for cond in ["candidateB_on", "sham"]:
...
def _write_manifest(analysis: dict) -> None:
    ...
    manifest: dict = {
        ...
        "candidate": "B (kin-sensing / collective organisation)",
        "regimes": {},
    }
...
def _print_famine_summary(famine: dict) -> None:
    ...
    print("\n  Kin-fraction at final step:")
    for cond in _CONDITIONS:
        s = summaries.get(cond, {})
        kf = s.get("kin_fraction_final", {})
        if kf.get("mean") is not None:
            print(f"    {cond:25s}  mean={kf['mean']:.4f}")
...
def run_analysis() -> dict:
    """Run full Candidate B stress-test analysis for all available regimes."""
    analysis: dict = {}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyse Candidate B (kin-sensing) stress-test experiment results."
    )
    parser.add_argument(
        "--no-figure",
        action="store_true",
